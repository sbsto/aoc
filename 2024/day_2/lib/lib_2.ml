...
let remove_one_level report =
  let rec aux acc = function
    | [] -> []
    | x :: xs -> (List.rev_append acc xs) :: (aux (x :: acc) xs)
  in
  aux [] report
...
let is_safe_with_removal report =
  if is_safe report then true
    ...
    let modified_reports = remove_one_level report in
    List.exists is_safe modified_reports
...
let count_safe_reports reports =
  let rec count_safe_reports acc reports =
    ...
    | [] -> acc
    | report :: reports ->
        if is_safe_with_removal report then
          count_safe_reports (acc + 1) reports
        else
          count_safe_reports acc reports
  in
